...
public class LoggingAspect {
	...
	@Before("execution(* com.abhi.Job_APP2.Service.jobservice.*(..))")
	public void beforeMethodCall(JoinPoint jp) {
        LOGGER.info("Method called: {}", jp.getSignature().getName());
	}

	@After("execution(* com.abhi.Job_APP2.Service.jobservice.*(..))")
	public void afterMethodCall(JoinPoint jp) {
        LOGGER.info("Method executed: {}", jp.getSignature().getName());
	}

	@AfterThrowing("execution(* com.abhi.Job_APP2.Service.jobservice.*(..))")
	public void afterException(JoinPoint jp) {
        LOGGER.error("Exception occurred in method: {}", jp.getSignature().getName());
	}

	@AfterReturning("execution(* com.abhi.Job_APP2.Service.jobservice.*(..))")
